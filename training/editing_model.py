...
class Decoder(nn.Module):
    ...
    def forward(self, style_code, mix_space=None, mask=None):
        ...
        if mix_space == None:
            ...
            reference_stylemaps = style_code[1] # shape : 1,64,8,8
            style_codes = []

            # 가장 낮은 layer 부터 stylemap을 생성한다.
            for up_layer in self.convs_latent:
                ...
                ratio = self.size // H
                # image_masks(1,1,256,256) -> masks(1,1,8,8) : stylemap size와 일치시킨다.
                ...
                current_mask = masks[0] > 0.5
                current_mask_moved = mask_moved[0] > 0.5
                original_stylemap[0][current_mask_moved] += weight * (
                    reference_stylemaps[0][current_mask]
                    - original_stylemap[0][current_mask_moved])

        out = self.input(batch)
        out = self.conv1(out, original_stylemap)
