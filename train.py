...
def save_img(batch, batch_images, path):
    images = []
    ...
    images = np.array(images).reshape(256*batch,256,3)
...
class DDPModel(nn.Module):
    ...
    def forward(self, real_img, mode,iter=None):
        if mode == "G":
            z = make_noise(
            ...
            fake_path = f'expr/check_result/{str(iter).zfill(6)}_fake_img.png'
            real_path = f'expr/check_result/{str(iter).zfill(6)}_real_img.png'

            save_img(self.args.batch_per_gpu,fake_img,fake_path)
            save_img(self.args.batch_per_gpu,real_img,real_path)
        ...
        with torch.no_grad():
            ...
            if i % 2500 == 0:
                ...
                for test_image in tqdm(val_loader):
                    ...
                    x_rec_loss, perceptual_loss = model(test_image, "cal_mse_lpips",i)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_network_interval", type=int, default=200022)
